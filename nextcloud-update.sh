...
NEXTCLOUD_OLD_INSTALL_FOLDER=`ls -d /usr/share/nginx/html/nextcloud-* | xargs -n 1 basename`
NEXTCLOUD_OLD_INSTALL_LOCATION="/usr/share/nginx/html/${NEXTCLOUD_OLD_INSTALL_FOLDER}"
...
fix_ownership_permissions_of_nextcloud()
{
  ...
  sudo chown -R $NGINX_USER:$NGINX_GROUP $NEXTCLOUD_DESTINATION_FOLDER
...
setup_new_nextcloud_version()
{
  ...
  sudo -u $NGINX_USER php occ maintenance:mode --on
  echo "Executing occ upgrade ..."
  sudo -u $NGINX_USER php occ upgrade
...
remove_old_nextcloud_folder()
{
  echo "Removing old Nextcloud installation folder - $NEXTCLOUD_OLD_INSTALL_LOCATION ..."
  rm -rf $NEXTCLOUD_OLD_INSTALL_LOCATION
}
